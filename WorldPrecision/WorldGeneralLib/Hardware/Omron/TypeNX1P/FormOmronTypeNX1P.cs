    ...
    public partial class FormOmronTypeNX1P : Form
    {
        ...
        private void ShowItems()
        {
            try
            {
                ...
                ipAddressControl1.Text = _plcData.RemoteIP;
                ipAddressControlPC.Text = _plcData.LocalIP;
            }
            catch (Exception)
            {
            }
        ...
        private void btnConn_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    if (!_plcDriver.omronFinsAPI.ConnectToOmronPLC(_plcData.RemoteIP, 9600,_plcData.LocalIP))
                    {
                        throw new Exception();
                    }
                }
            ...
            {
                MessageBox.Show("Failure to connected to PLC " + _plcData.Name,"Warning",MessageBoxButtons.OK, MessageBoxIcon.Warning, MessageBoxDefaultButton.Button1, MessageBoxOptions.ServiceNotification);
            }
            ...
            while(!MainModule.formMain.bExit)
            {
                ...
                try
                {
                    if(!_plcDriver.omronFinsAPI.bConnectOmronPLC)
                    {
                        Action action1 = () =>
                         {
                             labelConnSta.Text = "Failed to connect to PLC.";
                    ...
                    else
                    {
                        Action action2 = () =>
                        {
                            labelConnSta.Text = "Connect to PLC successfuly.";
        ...
        private void toolStripBtnExport_Click(object sender, EventArgs e)
        {
            ...
            MessageBox.Show("Successfully Exported.", "", MessageBoxButtons.OK, MessageBoxIcon.Warning, MessageBoxDefaultButton.Button1, MessageBoxOptions.ServiceNotification);
        }
        #endregion
        ...
        private void ipAddressControl1_TextChanged(object sender, EventArgs e)
        {
            try
            {
                if (null == _plcData)
                    return;
                _plcData.RemoteIP = ipAddressControl1.Text;
            }
            catch 
            {
            }
        ...
        private void ipAddressControlPC_TextChanged(object sender, EventArgs e)
        {
            try
            {
                if (null == _plcData)
                    return;
                _plcData.LocalIP = ipAddressControlPC.Text;
            }
            catch
            {
            }
