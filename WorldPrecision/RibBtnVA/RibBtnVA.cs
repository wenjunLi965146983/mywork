...
namespace RibBtnVA
{
    public partial class RibBtnVA : Button
    {
        ...
        private Color _titleForeColor = Color.FromArgb(0, 0, 0);
        ...
        public void FillGradients(Graphics gr, GraphicsPath pa)
        {
            ...
            if (_showbase == e_showbase.Yes)
            {
                ...
                #region Fill Band
                //rect = new Rectangle(new Point(0, 0), new Size(this.Width, this.Height / 2));
                //pa = new GraphicsPath();
                //int _rtemp = _radius;
                //_radius = _rtemp - 1;
                //DrawArc(rect, pa);
                ...
                //if (A0 > 80)
                //{
                //    gr.FillPath(new SolidBrush(Color.FromArgb(60, 255, 0, 0)), pa);
                //}
                //_radius = _rtemp;
                #endregion
                ...
                if (i_mode == 2)
                {
                    //if(this.FlatAppearance.BorderSize >= 0 && this.FlatAppearance.BorderColor != Color.Transparent)
                    {
                        rect = new Rectangle(1, 1, this.Width - 2, this.Height);
                        pa = new GraphicsPath();
                        DrawShadow(rect, pa);
                        gr.DrawPath(new Pen(this.FlatAppearance.BorderColor), pa);
                    }
                }
                else
                {
                    if (this.FlatAppearance.BorderSize > 0 && this.FlatAppearance.BorderColor != Color.Transparent)
                    {
                        ...
                        if (A0 > 80)
                        {
                            ...
                            //gr.DrawPath(new Pen(this.FlatAppearance.BorderColor, 3.0F), pa);
                        }
                    }
                }
                #endregion
                ...
                gr.DrawLine(pen, new Point(1, this.Height - 1), new Point(this.Width - 1, this.Height - 1));
                #endregion
                if (this.FlatAppearance.BorderSize > 0 && this.FlatAppearance.BorderColor != Color.Transparent)
                {
                    rect = new Rectangle(new Point(0, 0), new Size(this.Width - 1, this.Height - 1));
                    pa = new GraphicsPath();
                    DrawArc(rect, pa);
                    gr.DrawPath(new Pen(this.FlatAppearance.BorderColor, 0.9F), pa);
                }
            ...
            if (this.Text != "")
            {
                ...
                switch (_imagelocation)
                {
                    case e_imagelocation.Left:
                        if (this.Title != "")
                        {
                            gr.DrawString(this.Title, this.Font, new SolidBrush(_titleForeColor), new PointF(_textOffsetX + imagewidth + 4, this.Font.Size / 2 + _textOffsetY));
                            gr.DrawString(s1, fontb, new SolidBrush(this.ForeColor), new PointF(_textOffsetX + imagewidth + 4, 2 * this.Font.Size + 4 + _textOffsetY));
                            gr.DrawString(s2, fontb, new SolidBrush(this.ForeColor), new PointF(_textOffsetX + imagewidth + 4, 3 * this.Font.Size + 2 + _textOffsetY));
                        }
                        else
                        {
                            gr.DrawString(s1, fontb, new SolidBrush(this.ForeColor), new PointF(_textOffsetX + imagewidth + 4, this.Height / 2 - this.Font.Size + _textOffsetY));
                        }

                        break;
                    case e_imagelocation.Right:
                        gr.DrawString(this.Title, fontb, new SolidBrush(_titleForeColor), new PointF(_textOffsetX, this.Height / 2 - this.Font.Size + 1 - extraoffset + _textOffsetY));
                        gr.DrawString(this.Text, this.Font, new SolidBrush(this.ForeColor), new PointF(_textOffsetX, extraoffset + this.Height / 2 - this.Font.Size + _textOffsetY));
                        break;
                    case e_imagelocation.Top:
                        gr.DrawString(this.Text, this.Font, new SolidBrush(this.ForeColor), new PointF(_textOffsetX + this.Width / 2 - textwidth / 2 - 1, offsety + imageheight + _textOffsetY));
                        break;
                    case e_imagelocation.Bottom:
                        gr.DrawString(this.Text, this.Font, new SolidBrush(this.ForeColor), new PointF(_textOffsetX + this.Width / 2 - textwidth / 2 - 1, this.Height - imageheight - textheight + _textOffsetY));
                        break;
                    case e_imagelocation.None:
                        _forecolor = this.ForeColor;
                        gr.DrawString(this.Text, this.Font, new SolidBrush(this.ForeColor), new PointF(_textOffsetX + this.Width / 2 - textwidth / 2 - 1, this.Height / 2 - textheight / 2 + _textOffsetY));
                        break;
                    default:
                        break;
                }
            ...
            switch (_arrow)
            {
                case e_arrow.ToDown:
                    if (_imagelocation == e_imagelocation.Left)
                    {
                        ...
                        points[0] = new Point(this.Width - 8 * _size - _imageoffsetY, this.Height / 2 - _size / 2);
                        points[1] = new Point(this.Width - 2 * _size - _imageoffsetY, this.Height / 2 - _size / 2);
                        points[2] = new Point(this.Width - 5 * _size - _imageoffsetY, this.Height / 2 + _size * 2);
                        gr.FillPolygon(new SolidBrush(_forecolor), points);
                    }
                    else if (_imagelocation == e_imagelocation.Top)
                    {
                        Point[] points = new Point[3];
                        points[0] = new Point(this.Width / 2 + 8 * _size - _imageoffsetY, this.Height - _imageoffsetY - 5 * _size);
                        points[1] = new Point(this.Width / 2 + 2 * _size - _imageoffsetY, this.Height - _imageoffsetY - 5 * _size);
                        points[2] = new Point(this.Width / 2 + 5 * _size - _imageoffsetY, this.Height - _imageoffsetY - 2 * _size);
                        gr.FillPolygon(new SolidBrush(_forecolor), points);
                    }
                    break;
                case e_arrow.ToRight:
                    if (_imagelocation == e_imagelocation.Left)
                    {
                        int arrowxpos = this.Width - _splitdistance + 2 * _imageoffsetY;
                        Point[] points = new Point[3];
            ...
            else
            {
                ...
                if (_keeppress)
                {
                    ...
                    try
                    {
                        ...
                        {
                            if (typeof(RibBtnVA) == _control.GetType() & _control.Name != this.Name)
                            {
                                ((RibBtnVA)(_control))._ispressed = false;
                                ((RibBtnVA)(_control)).UpdateLeave();
                            }
                        }
                    }
                    catch { }
        ...
        protected override void OnClick(EventArgs e)
        {
            ...
            if(bIsClicked)
            {
                ...
                if(span.Seconds==0 &&  span.Milliseconds < SystemInformation.DoubleClickTime)
                {
                    DoubleClick(this,e);
                    bIsClicked = false;
                }
